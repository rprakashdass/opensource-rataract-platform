    ...
    <div className="min-h-screen bg-background pt-24 pb-16">
      <MaxWidthWrapper>
        <div className="space-y-16 max-w-4xl mx-auto">
          ...
          <div className="relative border-l border-primary/10 ml-4 md:ml-32 space-y-12 py-4">
            {HISTORICAL_MILESTONES.map((milestone, index) => {
               ...
               return (
                 ...
                 >
                   {/* Year Bubble on Left (Desktop) */}
                   <span className="hidden md:flex absolute right-full mr-12 top-1 text-2xl font-black bg-gradient-to-r from-primary to-purple-500 bg-clip-text text-transparent select-none">
                     {milestone.year}
                   </span>
 
                   {/* Bullet Node Icon */}
                   <span className={`absolute -left-4 top-1.5 flex h-8 w-8 items-center justify-center rounded-full ring-4 ring-background ${milestone.color}`}>
                     <Icon className="h-4 w-4" />
                   </span>
 
                   {/* Mobile Year Badge */}
                   <span className="flex md:hidden text-lg font-black bg-gradient-to-r from-primary to-purple-500 bg-clip-text text-transparent mb-1">
                     {milestone.year}
                   </span>
 
                   {/* Milestone Card */}
                   <div className="bg-card border border-primary/10 p-6 rounded-2xl space-y-2 hover:border-primary/20 hover:shadow-md transition-all">
