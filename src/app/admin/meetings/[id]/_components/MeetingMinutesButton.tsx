...
export default function MeetingMinutesButton({ meetingId }: { meetingId: string }) {
  const [sending, setSending] = useState(false);

  const handleSend = async () => {
    if (!confirm("This will email the meeting minutes to all members who attended. Are you sure?")) return;
    
    const toastId = toast.loading("Sending minutes emails...");
    setSending(true);
    try {
      const res = await fetch(`/api/admin/meetings/${meetingId}/minutes/email`, {
        method: "POST",
      });
